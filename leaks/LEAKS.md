...
Если вы читаете гайд со смартфона, то блоки кода можно прокручивать вправо, а картинки можно зумить.
...
На скриншоте видно, что в блоке defenetly lost в функции `main` был вызван `malloc`. Утекла именно та память, которую выделил этот маллок.
...
P.S. `valgrind` не "родной" для мака! Поэтому иногда **он может ошибаться**. Посмотрите [гайд amatilda](https://github.com/daniiomir/faq_for_school_21/blob/master/docs/memory_leaks_amatilda.pdf) в каких случаях валгринд считает за утечку то, что не является утечкой. 
...
Подробности этого поведения конечно же есть [тут](https://google.com).
