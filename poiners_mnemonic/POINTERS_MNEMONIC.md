...
```C
char   s1    // тип char
char  *s2    // тип указатель на char (адрес char'a)
char **s3    // тип указатель на указатель на char (адрес адреса char'а)
```
...
```C
float *ptr = 453;   // так делать законно, в переменной ptr хранится число 453

float b    = *ptr;  // скорее всего segmentation fault
```
...
у нас осталось `**ch`, туда то и нужно записывать символ.
...
у нас осталось `*ch`, туда и запишем адрес новой строки.
...
```C
do_smth_2(ch);         // отправили адрес адреса строки в другую функцию
```
